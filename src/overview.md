...
The parser uses the standard [`DiagnosticBuilder`] API for error handling, but
we try to recover, parsing a superset of Rust's grammar, while also emitting an
error. [`rustc_ast::ast`]::{[`Crate`], [`Expr`], [`Pat`], ...} AST nodes are
...
The HIR is further [lowered to Mid-level Intermediate Representation (MIR)][mir] 
...
representations of source are turned into an executable binary. Since `rustc`
...
is actually monomorphized.
The LLVM-IR is passed to LLVM, which does a lot more optimizations on it,
emitting machine code which is basically assembly code with additional
low-level types and annotations added (e.g. an ELF object or WASM). The
different libraries/binaries are then linked together to produce the final
binary.
