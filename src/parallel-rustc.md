...
- `Lrc`
    ...
    - [`Rc`][Rc] if it is not
- `MetadataRef`
    - The [`rustc` version of an OwningRef][OwningRef]

Once [monomorphization][monomorphization] has occurred, the compiler partitions
the monomorphized LLVM-IR into smaller chunks called _codegen units_. These units are then
sorted pairwise according to their relative size; i.e. the largest and smallest codegen units are first,
the second largest and second smallest are second, and so on. This was a compromise intended to
increase parallel throughput and decrease memory consumption. Initially codegen units
were sorted from largest to smallest; but this meant that all of the largest CGU's would be in memory at once.

The sorted units are then passed to independent instances of LLVM running in parallel
along with the `TyCtxt`. At the end, the linker is run to combine all the codegen
units together into one binary.
